//
//  ReadingConceptsViewController.swift
//  SAT Daily
//
...
class ReadingConceptsViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let docRef = db.collection("readingconcepts").document("video")
    ...
    func addStuff()
    {
        let resourcemax = UserDefaults.standard.string(forKey: "readingconceptmax")!
        ...
        for resourceNumber in 1...aor {
          let button = UIButton()
            ...
            var title = ""
            let docRef = db.collection("readingconcepts").document(String(resourceNumber))
            docRef.getDocument{ (document, error) in
                if let document = document, document.exists {
