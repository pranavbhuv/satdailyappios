...
class WritingConceptsViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let docRef = db.collection("writingconcepts").document("video")
    ...
    func addStuff()
    {
        let resourcemax = UserDefaults.standard.string(forKey: "writingconceptmax")!
        ...
        for resourceNumber in 1...aor {
          let button = UIButton()
            ...
            var title = ""
            let docRef = db.collection("writingconcepts").document(String(resourceNumber))
            docRef.getDocument{ (document, error) in
                if let document = document, document.exists {
