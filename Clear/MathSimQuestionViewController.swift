//
//  MathSimQuestionViewController.swift
//  SAT Daily
//
...
class MathSimQuestionViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if let mathValue = UserDefaults.standard.array(forKey: "mathArray") as? [Int], mathValue.contains(actualquestionid) {
    ...
    @IBAction func saveButton(_ sender: Any) {
        var arrayMath = UserDefaults.standard.array(forKey: "mathArray") as! [Int]
        if saveState == 0 {
            arrayMath.append(actualquestionid)
            UserDefaults.standard.set(arrayMath, forKey: "mathArray")
    ...
    func getInfoAndSet()
    {
        ...
        docRef.getDocument{ (document, error) in
            if let document = document, document.exists {
                ...
                self.question.text = "\(question)\n\n What is the answer yes to this problem?\n"
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if (control == 0)
        {
            let vc = segue.destination as! MathCorrectSimViewController
